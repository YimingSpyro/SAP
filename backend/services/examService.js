...
module.exports.getExamByModule = (data) => {
    return new Promise((resolve, reject) => {
        pool.query(`SELECT * FROM exam_verifier_sys WHERE fk_module_code = ? AND fk_semester_code = ?;`, data, (err, results) => {
            if (err) {
                reject(err);
            } else {
                if (results) {
...
module.exports.updateExam = (data) => {
    return new Promise((resolve, reject) => {
        ...
        pool.query(`UPDATE exam_verifier_sys
         ...
         WHERE fk_module_code = ? AND fk_semester_code = ?`, data, (err, results) => {
            if (err) {
                reject(err);
            } else {
                if (results) {
